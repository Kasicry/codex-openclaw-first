...
| 운영 지표 | Micrometer 메모리 지표와 Actuator metrics | 기존 Actuator 의존성을 활용하고 외부 모니터링 도입 전 기본선을 제공 |
| 실패 알림 | 기본 비활성화된 단일 webhook | 실제 수신 시스템 선정 전에도 고정된 최소 payload와 승인 경계를 검증 가능 |
| 기사 보존 | 365일 | 실제 증가율과 원문 보존 필요성이 확인되기 전의 초기 운영값 |
| 백업 보존 | 일별 PostgreSQL 논리 백업 30일 | 초기 운영 규모에서 복구 가능성과 저장 비용을 균형 있게 유지 |
| 일별 JSON 내보내기 | 보류 | PostgreSQL을 원본으로 유지하며 별도 소비자·복구 요구가 아직 없음 |
| 공유 요청 제한 | Backend 2개 이상부터 Redis 검토 | 현재 단일 인스턴스에는 프로세스 단위 제한이 충분함 |
| 운영 추세 스냅샷 | 집계 전용 JSON을 표준 출력으로 제공 | 승인된 외부 추세 저장소 없이 원문 노출과 파일 누적을 피하면서 반복 측정 가능 |
